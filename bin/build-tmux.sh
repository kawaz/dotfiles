#!/bin/sh
. "`dirname -- "$0"`"/functions.sh || exit

src="$DOTFILES_DIR/env/src/tmux"
dest="$DOTFILES_DIR/env/dest/tmux"

# 必要パッケージのチェック
has_rpm_packages automake libevent-devel ncurses-devel

# develop version
rm -rf "$src"
git clone git://tmux.git.sourceforge.net/gitroot/tmux/tmux "$src"
cd "$src" || exit 1

# 参考：罫線パッチ http://d.hatena.ne.jp/emonkak/20110521/1305970697
patch -p1 < "$DOTFILES_DIR/bin/build-tmux.sh.border_patch"

# libevent2が必要なので1.xしか無い場合はソースで入れる
if rpm -q libevent-devel-1.\* >/dev/null 2>&1; then
  ( src="$DOTFILES_DIR/env/src/libevent2"
    dest="$DOTFILES_DIR/env/dest/libevent2"
    mkdir -p "$src" && cd "$src" || exit 1
    curl -L https://github.com/downloads/libevent/libevent/libevent-2.0.20-stable.tar.gz | tar xz
    cd libevent-2.0.20-stable || exit 1
    ./configure --prefix="$dest" && make && make install
  ) || exit 1
  export CPPFLAGS="-I$dest/include"
  export LDFLAGS="-L$dest/lib"
  export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$dest/lib"
  # create startup script for livevent2
  ( echo "# generated by $DOTFILES_DIR/bin/$(basename -- "$0")"
    echo "export LD_LIBRARY_PATH=\"\$LD_LIBRARY_PATH:$dest/lib\""
  ) | make_profile_script 10-tmux-libevent2.sh
fi

sh autogen.sh || exit 1
./configure --prefix="$dest" && make && make install

# create startup script
( echo "# generated by $DOTFILES_DIR/bin/$(basename -- "$0")"
  echo "export PATH=\"$dest/bin:\$PATH\""
) | make_profile_script 10-tmux.sh
